...
const seeds = {
  'K-pop': () => {
    const seed_genres = 'k-pop';  // Only K-pop for a more specific result
    const target_valence = addRandomness(0.8);  // Positive, bright mood
    const target_energy = addRandomness(0.75); // High energy
    const target_tempo = 120;  // Faster tempo for upbeat vibe
    const target_acousticness = addRandomness(0.1);  // Low acousticness, more electronic
    const target_danceability = addRandomness(0.8);  // Very danceable
  ...
  'Jazz': () => {
    const seed_genres = 'cool jazz,jazz fusion,modal jazz,bebop';  // Jazz genres with similar vibes to Brubeck and Corea
    const target_valence = addRandomness(0.6);  // Moderate to upbeat, reflecting smooth yet dynamic jazz
    const target_energy = addRandomness(0.55); // Medium energy, both chill and dynamic
    const target_tempo = 110;  // A mix of moderate and upbeat tempos typical of jazz standards and fusion
    const target_acousticness = addRandomness(0.4);  // Moderate acousticness, leaning towards jazz but with some electric elements (fusion)
    const target_danceability = addRandomness(0.5);  // Smooth and rhythmic, but not overtly danceable
  ...
  'Top of the Charts': () => {
    const seed_genres = 'pop,hip hop,rap,dance pop,edm,r&b,synthpop';  // Popular genres in the USA charts
